...
VulkanRenderer::VulkanRenderer() : window(nullptr), isRunning(false), instance(nullptr), debugMessenger(0), surface(0) {
		isRunning = true;
}
...
SDL_Window* VulkanRenderer::createWindow(std::string name_, int width_, int height_)
{
	...
	window = SDL_CreateWindow(name_.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_VULKAN);
...
{
	//SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS);
	//window = SDL_CreateWindow("Vulkan Engine", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_VULKAN);
}
