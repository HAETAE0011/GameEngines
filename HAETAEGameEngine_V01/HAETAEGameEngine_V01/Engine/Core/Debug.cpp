...
void Debug::DebugInit(){
	outputName = "HAETAEEngineLog.txt";
	std::ofstream out;
...
void Debug::HardwareInfo(const std::string& message_, const GLubyte* message__){
	//Log(MessageType::TYPE_INFO, "[HardWareINFO]:" + message_, fileName_, line_);
		std::ofstream out;
		out.open(outputName.c_str(), std::ios::app | std::ios::out);
		out << message_ << message__ << std::endl;
		out.flush();
		out.close();
}
