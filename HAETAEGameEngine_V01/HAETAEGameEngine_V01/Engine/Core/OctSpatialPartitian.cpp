...
void OctNode::Octify(int depth_)
{
	if (depth_ > 0 && this) {
		...
		children[static_cast<int>(OctChildren::OCT_BLF)] = new OctNode(glm::vec3(octBounds->minVert.x, octBounds->minVert.y + half, octBounds->minVert.z), half, this);
		children[static_cast<int>(OctChildren::OCT_BRF)] = new OctNode(glm::vec3(octBounds->minVert.x + half, octBounds->minVert.y + half, octBounds->minVert.z), half, this);
		...
		children[static_cast<int>(OctChildren::OCT_TLR)] = new OctNode(glm::vec3(octBounds->minVert.x, octBounds->minVert.y, octBounds->minVert.z + half), half, this);
		...
		children[static_cast<int>(OctChildren::OCT_TRR)] = new OctNode(glm::vec3(octBounds->minVert.x + half, octBounds->minVert.y, octBounds->minVert.z + half), half, this);
		//fill other childerens
...
void OctSpatialPartition::PrepareCollisionQuery(OctNode* cell_, Ray ray)
{
	if (cell_ != nullptr) {
		BoundingBox* box = cell_->GetBoundingBox();
		if (ray.IsColliding(box)) {
			if (cell_->IsLeaf()) {
