...
Ray CollisionDetection::ScreenPosToWorldRay(glm::vec2 mouseCoords_, glm::vec2 screenSize_, Camera* camera_) {
	...
	glm::vec4 rayStart_NDC((mouseCoords_.x / screenSize_.x - 0.5f) * 2.0f, (mouseCoords_.y / screenSize_.y - 0.5f) * 2.0, -1.0f, 1.0f);
	glm::vec4 rayEnd_NDC((mouseCoords_.x / screenSize_.x - 0.5f) * 2.0f, (mouseCoords_.y / screenSize_.y - 0.5f) * 2.0, 0.0f, 1.0f);
...
bool CollisionDetection::RayObbIntersection(Ray* ray_, BoundingBox* box_) {
	float tMin = CoreEngine::GetInstace()->getCamera()->GetClippingPlanes().x;
	float tMax = CoreEngine::GetInstace()->getCamera()->GetClippingPlanes().y;
	...
	if (fabs(f1) > 0.001f) {
		float t1 = (e1 + box_->minVert.y) / f1;
		float t2 = (e1 + box_->maxVert.y) / f1;
	...
	if (fabs(f2) > 0.001f) {
		float t1 = (e2 + box_->minVert.z) / f2;
		float t2 = (e2 + box_->maxVert.z) / f2;
